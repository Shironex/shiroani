...
export class ProviderSyncEngine<R extends RemoteEntryCore> {
   ...
   * resolves with the final tally. The caller is responsible for the single-flight
   * guard and the connected-account check (the adapter's {@link SyncProviderAdapter.getViewerId}
   * throws if not connected).
   */
  async runFullSync(onProgress: (progress: SyncProgress) => void): Promise<SyncResult> {
    ...
    // A provider can return the same media more than once (AniList returns a media
    // per list it belongs to). Dedup by provider id — keeping the most recently
    // updated occurrence — so each media is reconciled exactly once and a
    // remote-only entry is never double-inserted against the UNIQUE id constraint.
    ...
    const localById = new Map<number, (typeof localEntries)[number]>();
    ...
    const report = (title: string, action: SyncAction): void => {
      current += 1;
    ...
    for (const remote of remoteUnique) {
      try {
        ...
        const fresh = this.library.getEntryById(local.id);
        if (!fresh || fresh.updatedAt !== local.updatedAt) {
