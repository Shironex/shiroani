...
describe('ColorPickerField', () => {
  ...
  it('reflects the current value as a hex string in the text input', () => {
    setup();
    const hexInput = screen.getByRole('textbox', { name: 'Kod heksadecymalny: Primary' });
  ...
  it('clicking the swatch button opens the hidden native color input', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Wybierz kolor: Primary' }));
    expect(clickSpy).toHaveBeenCalled();
  });

  it('exposes accessible names for both the swatch button and the hex input', () => {
    setup();
    expect(screen.getByRole('button', { name: 'Wybierz kolor: Primary' })).toBeInTheDocument();
    expect(
