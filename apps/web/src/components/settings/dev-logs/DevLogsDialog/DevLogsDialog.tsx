...
import { useDevLogsDialog } from './DevLogsDialog.hooks';
import type { IDevLogsDialogProps } from './DevLogsDialog.types';

export default function DevLogsDialog({ open, onOpenChange }: IDevLogsDialogProps) {
  const { t } = useTranslation('settings');
  const view = useDevLogsDialog({ open });
  const { logs, stickyTail, source, filteredEntries, fileTotalCount, totalCountForHeader } = view;
  ...
  const showTailButton = stickyTail.showJumpToTail && logs.hasFilteredEntries;
        ...
        <div className="relative">
          ...
          >
            {logs.fileLoading && source !== 'buffer' ? (
              ...
              <ul className="divide-y divide-border-glass/50">{entryRows}</ul>
            )}
          </div>
