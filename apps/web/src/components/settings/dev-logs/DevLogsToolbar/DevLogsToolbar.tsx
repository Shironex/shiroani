...
import { useDevLogsToolbar } from './DevLogsToolbar.hooks';
import { SourceButton } from './DevLogsToolbar.parts';
import type { IDevLogsToolbarProps } from './DevLogsToolbar.types';
...
export default function DevLogsToolbar({ logs }: IDevLogsToolbarProps) {
  const { t } = useTranslation('settings');
  useDevLogsToolbar();
  const { source } = logs;

  const archiveItems = logs.fileList.map(file => (
      ...
      {source === 'archive' && (
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <SelectContent>{archiveItems}</SelectContent>
          </Select>
        </div>
      )}
