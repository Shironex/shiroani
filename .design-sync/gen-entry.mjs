...
const stories = Object.values(idx.entries ?? idx.stories ?? {}).filter((e) => e.type === 'story' || e.type === 'docs' ? e.type === 'story' : true);
...
function resolveSpec(spec, fromFile) {
  ...
  else if (spec.startsWith('@shiroani/')) return null; // workspace pkg, not a local component
...
for (const [title, importPath] of byTitle) {
  ...
  if (!/^[A-Z][A-Za-z0-9]*$/.test(name)) { skipped.push([title, 'name not PascalCase']); continue; }
  if (seenNames.has(name)) { skipped.push([title, `duplicate name ${name}`]); continue; }

  const storyFile = resolve(WEB, importPath.replace(/^\.\//, ''));
  if (!existsSync(storyFile)) { skipped.push([title, `story file missing: ${importPath}`]); continue; }
  ...
  let spec = null, isDefault = false, importedName = compId;
  // default import: `import Foo from '...'`
  const defRe = new RegExp(`import\\s+${compId}\\s*(?:,\\s*\\{[^}]*\\})?\\s+from\\s+['"]([^'"]+)['"]`);
  const defM = code.match(defRe);
  if (defM) { spec = defM[1]; isDefault = true; }
  if (!spec) {
    // named import: `import { ..., Foo, ... } from '...'` (also `Foo as Bar`)
    const namedRe = new RegExp(`import\\s+(?:[A-Za-z_$][\\w$]*\\s*,\\s*)?\\{([^}]*\\b${compId}\\b[^}]*)\\}\\s+from\\s+['"]([^'"]+)['"]`);
  ...
  if (!spec) { skipped.push([title, `no import found for ${compId}`]); continue; }

  const modPath = resolveSpec(spec, storyFile);
  if (!modPath) { skipped.push([title, `unresolvable spec ${spec}`]); continue; }
  let rel = relative(OUT_DIR, modPath).replace(/\\/g, '/').replace(/\.(tsx|ts|jsx|js)$/, '');
...
writeFileSync(OUT,
  ...
  exportsList.join('\n') + '\n' +
  [...starExports].sort().map((r) => `export * from '${r}';`).join('\n') + '\n');
writeFileSync(join(OUT_DIR, 'package.json'),
  JSON.stringify({ name: '@shiroani/web-ds', private: true, type: 'module', types: './index.tsx', module: './index.tsx' }, null, 2) + '\n');
